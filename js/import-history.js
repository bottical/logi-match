...
async function init(){try{const ctx=await window.requireAppContext('import-history');if(!ctx)return;console.log('[import-history] context',{uid:ctx.uid||ctx.userId,clientId:ctx.clientId,role:ctx.role});window.renderSidebar?.();await loadImportHistory(ctx);}catch(e){console.error('[import-history] init failed',e);showInitError(e);}}
