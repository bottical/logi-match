(function(){
  ...
  function normalizeRole(role) { return role === 'operator' ? WORKER_ROLE : role; }
  function isAdminRole(role) { return ADMIN_ROLES.includes(normalizeRole(role)); }
  ...
  window.renderSidebar = function(){
    ...
    const page=document.body.dataset.page||''; const role=normalizeRole(window.appContext?.role); if(!role) return;
    const items=[
      { id:'inspection',href:'./#inspection',label:'検品実行',roles:['owner','admin',WORKER_ROLE]},
      ...
      { id:'unstarted-list',href:'./#unstarted-list',label:'検品未着手一覧',roles:['owner','admin',WORKER_ROLE]},
      ...
      { id:'internal-users',href:'./#internal-users',label:'ユーザー管理（弊社専用）',roles:['systemOwner'],internalOnly:true}
    ].filter((i)=> i.internalOnly ? (typeof window.isInternalAdmin === 'function' && window.isInternalAdmin(window.appContext)) : (role===WORKER_ROLE ? i.roles.includes(WORKER_ROLE) : (isAdminRole(role) ? i.roles.includes(role)||i.roles.includes(WORKER_ROLE) : i.roles.includes(role))));
