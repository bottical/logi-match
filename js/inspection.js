...
function render(){
 ...
 $('skuProgress').textContent=`${skuDone} / ${state.details.length}`;$('qtyProgress').textContent=`${qa} / ${qt}`;$('judgeProgressText').textContent=state.details.length?`数量 ${qa} / ${qt}`:'';
 ...
 DETAIL_TABLE_COLUMNS.forEach(c=>{const td=document.createElement('td');if(c.type==='state')td.textContent=d.completed_flag?'完了':(d.actual_qty>0?'途中':'未着手');else if(c.type==='target')td.textContent=String(d.target_qty||0);else if(c.type==='actual')td.textContent=String(d.actual_qty||0);else if(c.type==='remaining')td.textContent=String((d.target_qty||0)-(d.actual_qty||0));else if(c.type==='jan4'){const jan=String(pickValue(d,['main_barcode','scan_code','barcode','JAN'])||'');td.textContent=jan?jan.slice(-4):'-';td.title=jan||'';}else td.textContent=display(pickValue(d,[c.key,...(c.fallbackKeys||[])]));tr.appendChild(td);});body.appendChild(tr);});
...
async function runScan(){ if(!selectedWorker()){setJudge('warning','作業者を選択してください','');playSound('warning');focusPickingNoInput();return;}if(!state.work.work_id||$('scanCodeInput').disabled){setJudge('warning','ピッキングNo.待ち','次のピッキングNo.をスキャンしてください');focusPickingNoInput();return;}const raw=$('scanCodeInput').value.trim();if(!raw)return;$('scanCodeInput').value='';const qty=Number($('scanQtyInput').value);
 if(!Number.isInteger(qty)||qty<=0){await safeAppendScanLog(state.work.work_id,{jan:raw,qty:0,workerId:selectedWorker().workerId,workerName:selectedWorker().workerName,loginUid:window.appContext.uid,loginEmail:window.appContext.email,result:'ng',message:'数量不正'});setJudge('error','数量エラー','読取数量を確認してください');playSound('ng');$('scanQtyInput').value='1';focusJanInput();return;}
 const detail=state.details.find(d=>d.scan_code===raw||d.main_barcode===raw||d.alt_code===raw);if(!detail){await safeAppendScanLog(state.work.work_id,{jan:raw,qty:0,workerId:selectedWorker().workerId,workerName:selectedWorker().workerName,loginUid:window.appContext.uid,loginEmail:window.appContext.email,result:'ng',message:'対象外JAN'});setJudge('error','対象外JANです',`読取JAN：${raw}`);playSound('ng');$('scanQtyInput').value='1';focusJanInput();return;}if(detail.actual_qty+qty>detail.target_qty){await safeAppendScanLog(state.work.work_id,{jan:raw,qty:0,workerId:selectedWorker().workerId,workerName:selectedWorker().workerName,loginUid:window.appContext.uid,loginEmail:window.appContext.email,result:'ng',message:'必要数超過'});setJudge('error','必要数を超えています','数量を確認してください');playSound('ng');$('scanQtyInput').value='1';focusJanInput();return;}
 await safeAppendScanLog(state.work.work_id,{jan:raw,qty,workerId:selectedWorker().workerId,workerName:selectedWorker().workerName,loginUid:window.appContext.uid,loginEmail:window.appContext.email,result:'ok',message:''});
 detail.actual_qty+=qty;detail.completed_flag=detail.actual_qty===detail.target_qty;state.lastScannedCode=raw;state.recentScan={scanCode:raw,detail,at:Date.now()};const remain=(detail.target_qty||0)-(detail.actual_qty||0);
 setJudge('ok',`OK ${detail.product_name||''} +${qty}`,`数量 ${detail.actual_qty} / ${detail.target_qty}  残り ${remain}`);
 const allDone=state.details.every(d=>d.completed_flag);if(allDone){state.work.status='completed';state.work.completed_flag=true;state.work.completed_at=new Date().toISOString();state.work.completed_worker_id=selectedWorker().workerId;state.work.completed_worker_name=selectedWorker().workerName;state.work.completed_login_uid=window.appContext.uid;state.work.completed_login_email=window.appContext.email;state.work.current_worker_id=null;state.work.current_worker_name=null;state.work.current_login_uid=null;state.work.current_login_email=null;state.lastCompletedSummary=`${state.work.work_id} / SKU ${state.details.length}件 / 数量 ${state.details.reduce((n,d)=>n+d.target_qty,0)}点`;playSound('strong-complete');render();$('scanQtyInput').value='1';const ok=await persistAsync('scan',{scanCode:raw,detailId:detail.detail_id,qtyDelta:qty});if(ok)resetToPickingNoInput({completed:true,completedSummary:state.lastCompletedSummary});return;}
