...
async function init(){resetToPickingNoInput();$('headerUserName')?.remove();
 ...
 const updateWorkerUi=(worker)=>{ $('workerDisplayName').textContent=worker?.workerName||'未選択'; $('scanCodeInput').disabled=true; $('scanQtyInput').disabled=true; if(worker){ setJudge('idle','ピッキングNo.待ち','ピッキングNo.を入力してください'); render(); focusPickingNoInput(); }else{ setJudge('warning','作業者を選択してください','右上の「変更」から作業者を選択してください'); render(); } };
 ...
 workerSelect?.addEventListener('blur',()=>{ if(selectedWorker()?.workerId){ workerSelect.hidden=true; $('workerChangeButton').hidden=false; }});
