...
const isShellEmbedded=()=>document.documentElement.classList.contains('shell-embedded-root');
...
async function init(){resetToPickingNoInput();$('headerUserName')?.remove();
 ...
 workerSelect?.addEventListener('change',()=>applyWorkerSelection(workerSelect.value,{fromChange:true}));
 requiredSelect?.addEventListener('change',()=>{ if(workerSelect) workerSelect.value=requiredSelect.value; if(requiredMessage) requiredMessage.textContent=''; });
 $('workerRequiredConfirmButton')?.addEventListener('click',confirmSelection);
 requiredSelect?.addEventListener('keydown',e=>{ if(e.key==='Enter'){ e.preventDefault(); confirmSelection(); }});
 $('workerChangeButton').addEventListener('click',()=>{ if(requiredPanel) requiredPanel.hidden=false; if(requiredSelect) requiredSelect.value=selectedWorker()?.workerId||''; if(requiredMessage) requiredMessage.textContent=''; (requiredSelect||workerSelect)?.focus(); });
